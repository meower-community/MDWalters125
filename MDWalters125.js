...
const help = ["~hello", "~help", "~str", "~amazing", "~uptime", "~uwu", "~8ball", "~motd", "~restart", "~shutdown", "~ulist", "~zen", "~github"];
...
window.handlePost = async function(bundle) {
    ...
    if (bundle[1].startsWith("~github")) {
        var user = await fetchURL(`https://api.github.com/users/${bundle[1].split(" ")[1]}`);
        var user = JSON.parse(user);
        post(`${user.login}: ${user.bio}`);
    }
}
