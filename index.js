...
fs.writeFile('logs.txt', [user] + ' tried to use the command ' + [message] + ', but they are muted from ' + [username] + '\n', { flag: 'a+' }, err => {});
    ...
    if (message.startsWith("~") && !(help.includes(message.split(" ")[0]))) {
        ...
        fs.writeFile('logs.txt', [user] + ' tried to use a command that does not exist. The command was ' + [message] + '\n', { flag: 'a+' }, err => {});
        return;
    }
    ...
    Create and answer polls.`, origin);
                ...
                fs.writeFile('logs.txt', [user] + ' tried to get help on a command that does not exist. The command was ' + [message] + '\n', { flag: 'a+' }, err => {});
            }
        }
    }
    ...
    if (message.startsWith("~uptime")) {
        ...
        fs.writeFile('logs.txt', [user] + ' used the command ' + [message] + '\n', { flag: 'a+' }, err => {});
    }
    ...
    if (message.startsWith("~status")) {
        if (message.split(" ")[1] === "set") {
            ...
            fs.writeFile('logs.txt', [user] + ' set their status with the command ' + [message] + '\n', { flag: 'a+' }, err => {});
        } else if (message.split(" ")[1] === "clear") {
            db.delete(`MDW125-STATUS-${user}`);
            bot.post("Status successfully cleared!", origin);
            fs.writeFile('logs.txt', [user] + ' cleared their status with the command ' + [message] + '\n', { flag: 'a+' }, err => {});
            ...
            if (message.split(" ")[2] === user) {
                if (!(db.has(`MDW125-STATUS-${user}`))) {
                    ...
                    fs.writeFile('logs.txt', [user] + " tried to view their status, but they don't have one set. They used the command " + [message] + '\n', { flag: 'a+' }, err => {});
                } else {
                    bot.post(`Your status:
    ${db.get("MDW125-STATUS-" + user)}`, origin);
    fs.writeFile('logs.txt', [user] + ' viewed their status with the command ' + [message] + '\n', { flag: 'a+' }, err => {});
                }
            } else {
                if (db.has(`MDW125-STATUS-${user}`)) {
                    bot.post(`@${message.split(" ")[2]}'s status:
    ${db.get("MDW125-STATUS-" + message.split(" ")[2])}`, origin);
    fs.writeFile('logs.txt', [user] + " viewed someone else's status with the command " + [message] + '\n', { flag: 'a+' }, err => {});
                } else {
                    bot.post(`@${message.split(" ")[2]} doesn't have a status set.`, origin);
                    fs.writeFile('logs.txt', [user] + " tried to view someone else's status, but they don't have one set. They used the command " + [message] + '\n', { flag: 'a+' }, err => {});
                }
            }    
        } else {
            if (!(db.has(`MDW125-STATUS-${user}`))) {
...
fs.writeFile('logs.txt', [user] + ' used the command ' + [message] + '\n', { flag: 'a+' }, err => {});
    	...
    	if (message.split(" ")[1] === "upvote") {
            if (!(db.has(`MDW125-KARMA-${message.split(" ")[2]}`))) {
                if (message.split(" ")[2] === user) {
                    ...
                    fs.writeFile('logs.txt', [user] + ' tried to upvote themselves unsucessfully with the command  ' + [message] + '\n', { flag: 'a+' }, err => {});
                } else {
                    db.set(`MDW125-KARMA-${message.split(" ")[2]}`, 1);
                    bot.post(`Successfully upvoted @${message.split(" ")[2]}! They now have 1 karma.`, origin);
                    fs.writeFile('logs.txt', [user] + ' upvoted someone with the command ' + [message] + '\n', { flag: 'a+' }, err => {});
                }
            } else {
                if (message.split(" ")[2] === user) {
                    bot.post("You can't upvote yourself!", origin);
            ...
            if (!(db.has(`MDW125-KARMA-${message.split(" ")[2]}`))) {
                if (message.split(" ")[2] === user) {
                    ...
                    fs.writeFile('logs.txt', [user] + ' tried to downvote themselves unsucessfully with the command  ' + [message] + '\n', { flag: 'a+' }, err => {});
                } else {
                    db.set(`MDW125-KARMA-${message.split(" ")[2]}`, -1);
                    bot.post(`Successfully downvoted @${message.split(" ")[2]}. They now have -1 karma.`, origin);
                    fs.writeFile('logs.txt', [user] + ' downvoted someone with the command ' + [message] + '\n', { flag: 'a+' }, err => {});
                }
            } else {
                if (message.split(" ")[2] === user) {
                    bot.post("You can't downvote yourself!", origin);
            ...
            if (message.split(" ")[2] === user) {
                if (!(db.has(`MDW125-KARMA-${user}`))) {
                    ...
                    fs.writeFile('logs.txt', [user] + ' viewed their 0 karma with the command ' + [message] + '\n', { flag: 'a+' }, err => {});
                } else {
                    bot.post(`You have ${db.get("MDW125-KARMA-" + user)} karma.`, origin);
                    fs.writeFile('logs.txt', [user] + ' viewed their karma with the command ' + [message] + '\n', { flag: 'a+' }, err => {});
                }
            } else {
                if (!(db.has(`MDW125-KARMA-${message.split(" ")[2]}`))) {
                    bot.post(`@${message.split(" ")[2]} has 0 karma.`, origin);
                    fs.writeFile('logs.txt', [user] + " viewed someone else's 0 karma with the command " + [message] + '\n', { flag: 'a+' }, err => {});
                } else {
                    bot.post(`@${message.split(" ")[2]} has ${db.get("MDW125-KARMA-" + message.split(" ")[2])} karma.`, origin);
                    fs.writeFile('logs.txt', [user] + " viewed someone else's karma with the command " + [message] + '\n', { flag: 'a+' }, err => {});
                }
            }
        } else {
            if (!(db.has(`MDW125-KARMA-${user}`))) {
    ...
    if (message.startsWith("~mute")) {
        if (admins.includes(user)) {
            if (db.has(`MDW125-MUTED-${message.split(" ")[1]}`)) {
                ...
                fs.writeFile('logs.txt', [user] + " tried to mute someone, but they are already muted. They used the command " + [message] + '\n', { flag: 'a+' }, err => {});
            } else {
                if (message.split(" ")[2]) {
                ...
                fs.writeFile('logs.txt', [user] + " muted someone with the command " + [message] + '\n', { flag: 'a+' }, err => {});
            }
        } else {
            bot.post("You don't have the permissions to run this command.", origin);
            fs.writeFile('logs.txt', [user] + " tried to mute someone, but they didn't have permission to do so. They used the command " + [message] + '\n', { flag: 'a+' }, err => {});
        }
    }
    ...
    if (message.startsWith("~unmute")) {
        if (admins.includes(user)) {
            if (!(db.has(`MDW125-MUTED-${message.split(" ")[1]}`))) {
                ...
                fs.writeFile('logs.txt', [user] + " tried to unmute someone, but they weren't muted. They used the command " + [message] + '\n', { flag: 'a+' }, err => {});
            } else {
                db.delete(`MDW125-MUTED-${message.split(" ")[1]}`);
                bot.post(`Successfully unmuted @${message.split(" ")[1]}!`, origin);
                fs.writeFile('logs.txt', [user] + " unmuted someone with the command " + [message] + '\n', { flag: 'a+' }, err => {});
            }
        } else {
            bot.post("You don't have the permissions to run this command.", origin);
            fs.writeFile('logs.txt', [user] + " tried to unmute someone, but they didn't have permission to do so. They used the command " + [message] + '\n', { flag: 'a+' }, err => {});
        }
    }
    ...
    if (message.startsWith("~wordle")) {
        if (message.split(" ")[1] === "new") {
            ...
            fs.writeFile('logs.txt', [user] + " started a Wordle game with the command " + [message] + '\n', { flag: 'a+' }, err => {});
...
`, origin);
    ...
    if (message.startsWith("~poll")) {
        if (message.split(" ")[1] === "new") {
            ...
            fs.writeFile('logs.txt', [user] + " created a new poll with the command " + [message] + '\n', { flag: 'a+' }, err => {});
        } else if (message.split(" ")[1] === "answer") {
            let polls = db.get("MDW125-POLLS");
            if (user == polls[message.split(" ")[2] - 1]) {
                bot.post("You can't answer a poll you made!", origin);
                fs.writeFile('logs.txt', [user] + " tried to answer a poll they created with the command " + [message] + '\n', { flag: 'a+' }, err => {});
            } else {
                ...
                fs.writeFile('logs.txt', [user] + " answered a poll with the command " + [message] + '\n', { flag: 'a+' }, err => {});
            }
        } else {
            let polls = db.get("MDW125-POLLS");
    ...
    fs.writeFile('logs.txt', [user] + " found a random poll with the command " + [message] + '\n', { flag: 'a+' }, err => {});
        }
    }
});
...
bot.onLogin(() => {
    ...
    fs.writeFile('logs.txt', "Logged on as user " + [username] + '\n', { flag: 'a+' }, err => {});
});
