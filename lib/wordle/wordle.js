...
export default class Wordle {
    ...
    init() {
        this.wordList = JSON.parse(fs.readFileSync("./lib/wordle/words.json", "utf8"));
        this.word = this.wordList[Math.floor(Math.random() * data.length)].toLowerCase();
        this.tries = -1;
        this.ready = true;
        this.grid = [
            ["⬜", "⬜", "⬜", "⬜", "⬜"],
    ...
    guess(word) {
        ...
        if (this.wordList[word.toLowerCase()] === undefined) {
            throw "That word is invalid! Please check the spelling, or try another word.";
            return;
        }
